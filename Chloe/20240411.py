...
def fetch_tickers(api_key):
    ...
    while api_url:
        ...
        if tickerdata.get('next_url'):
            api_url = tickerdata.get('next_url') + f'&apiKey={api_key}'  
        else:
           break
...
#tickers_df.to_csv("/Repo_ACCPythonAnalytics202403/Chloe/tickers.csv", index=False)

#test aggr load process

ticketlist = [‘ABT”,’ RHHBY’,'PFE','BYND','PSTX']
    ...
    match opmode:
        case 'full':
            ...
            # Convert the list of tickeraggr to a DataFrame
              ...
              df_tickeraggr.to_csv(f"/workspaces/Repo_ACCPythonAnalytics202403/Chloe/stockhistory/{t}.csv", index=False)
